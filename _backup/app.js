...
class AssetTrackingApp {
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("incident-filter")
    ...
    // Reports
    document
      .getElementById("schedule-report")
    ...
    document
      .getElementById("close-asset-details")
      ?.addEventListener("click", () => {
  ...
  updateKPICards() {
    ...
    document.getElementById("devices-offline").textContent =
  ...
  initializeMap() {
    ...
    this.map = L.map("map").setView([3.139, 101.6869], 10);
  ...
  renderActivityStream() {
    ...
    const sortedEvents = this.data.events.sort(
      (a, b) => new Date(b.time) - new Date(a.time),
    );

    activityList.innerHTML = sortedEvents
      .map(
        (event) => `
  ...
  renderAssetsTable() {
    ...
    tableBody.innerHTML = filteredAssets
      .map(
        (asset) => `
            <tr>
                <td>${asset.id}</td>
  ...
  renderDevicesTable() {
    ...
    const unassignedCount = this.data.devices.filter(
      (d) =>
        d.available > 0 &&
        (!d.assigned_assets || d.assigned_assets.length === 0),
    ).length;
  ...
  renderCameraGrid() {
    ...
    cameraGrid.innerHTML = cameraAssets
      .map(
        (asset) => `
  ...
  renderIncidentsList() {
    ...
    incidentsList.innerHTML = filteredIncidents
      .map(
        (incident) => `
  ...
  renderAnalyticsCharts() {
    ...
    const assetStatusCtx = document
      .getElementById("asset-status-chart")
      ?.getContext("2d");
    if (assetStatusCtx) {
    ...
    const deviceHealthCtx = document
      .getElementById("device-health-chart")
      ?.getContext("2d");
    if (deviceHealthCtx) {
    ...
    const incidentsCtx = document
      .getElementById("incidents-chart")
      ?.getContext("2d");
    if (incidentsCtx) {
  ...
  renderReports() {
    ...
    reportsGrid.innerHTML = this.data.report_templates
      .map(
        (report) => `
  ...
  addAsset() {
    ...
    const selectedDevices = Array.from(
      document.querySelectorAll('input[name="devices"]:checked'),
    ).map((cb) => cb.value);
    ...
    const newAsset = {
      ...
      lat: 3.139, // Default coordinates for Malaysia
      lng: 101.6869,
    };
  ...
  addDevice() {
    ...
    const quantity = parseInt(
      document.getElementById("new-device-quantity").value,
    );
    const firmware =
      document.getElementById("new-device-firmware").value.trim() || "1.0.0";
    ...
    const modalContent = `
            <div class="add-driver-form">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">Status</label>
                        <select class="form-control" id="new-driver-status" disabled>
                              ...
                              .filter(
                                (a) =>
                                  !this.data.drivers.find(
                                    (d) => d.current_assignment === a.id,
                                  ),
                              )
                              .map(
                                (a) =>
                                  `<option value="${a.id}">${a.id} - ${a.make} ${a.model}</option>`,
                              )
                              .join("")}
                        </select>
                    </div>
  ...
  addDriver() {
    ...
    const safetyScore =
      parseFloat(document.getElementById("new-driver-score").value) || 4.0;
    const assignedAsset = document.getElementById("new-driver-asset").value;
    const status = assignedAsset ? "On Duty" : "Available";
  ...
  showAssetDetails(assetId) {
    ...
    const devices = asset.assigned_devices
      ? this.data.devices.filter((d) => asset.assigned_devices.includes(d.id))
      : [];
    const driver = this.data.drivers.find(
      (d) => d.name === asset.current_driver,
    );
                        ...
                        ${
                          devices.length > 0
                            ? devices
                                .map(
                                  (d) =>
                                    `<span class="device-tag">${d.id} (${d.type})</span>`,
                                )
                                .join("")
                            : "No devices assigned"
                        }
                    ...
                    <select class="form-control" id="driver-assignment">
                        ${
                          this.data.drivers.filter(
                            (d) =>
                              d.status === "Available" || !d.current_assignment,
                          ).length > 0
  ...
  confirmAuthorization(assetId) {
    ...
    if (!driver) {
      this.showToast("Please select a driver", "error");
      return;
    }
    ...
    if (asset.current_driver && asset.current_driver !== driver) {
      const oldDriver = this.data.drivers.find(
        (d) => d.name === asset.current_driver,
      );
      if (oldDriver) {
        oldDriver.current_assignment = null;
    ...
    const driverObj = this.data.drivers.find((d) => d.name === driver);
    if (driverObj) {
      driverObj.current_assignment = assetId;
      driverObj.status = "On Duty";
    }
  ...
  assignDevice(deviceId) {
    ...
    if (!device || device.available <= 0) {
      this.showToast(
        "No devices of this type available for assignment",
        "error",
      );
      return;
    }
    ...
    const availableAssets = this.data.assets.filter((a) => {
      const assignedDevicesCount = a.assigned_devices
        ? a.assigned_devices.length
        : 0;
    ...
    if (availableAssets.length === 0) {
      this.showToast(
        "All assets have maximum devices assigned (3 devices per asset)",
        "error",
      );
      return;
    }
            ...
            <div class="device-assignment">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="asset-assignment">
                        ${availableAssets
                          .map((a) => {
                            const deviceCount = a.assigned_devices
                              ? a.assigned_devices.length
                              : 0;
  ...
  confirmCustomScheduleReport() {
    ...
    this.closeModal();
    this.showToast(
      `${reportType} scheduled ${frequency} starting ${startDate}`,
      "success",
    );
  }
  ...
  confirmDeviceAssignment(deviceId) {
    ...
    const currentDeviceCount = asset.assigned_devices
      ? asset.assigned_devices.length
      : 0;
  ...
  resolveIncident(incidentId, event) {
    ...
    this.data.kpi_stats.critical_alerts = Math.max(
      0,
      this.data.kpi_stats.critical_alerts - 1,
    );
  ...
  viewDriverProfile(driverName) {
    ...
    const driverIncidents = this.data.incidents.filter(
      (i) => i.related === driver.current_assignment,
    );
    ...
    const modalContent = `
            <div class="driver-profile">
                <div class="driver-header">
                    <h3>${driver.name}</h3>
                    <span class="status status--${this.getDriverStatusClass(driver.current_assignment ? "On Duty" : "Available")}">${driver.current_assignment ? "On Duty" : "Available"}</span>
                </div>
                <div class="driver-details">
                    ...
                    <div class="detail-section">
                        ...
                        <div><strong>Status:</strong> ${driver.current_assignment ? "On Duty" : "Available"}</div>
                    </div>
                </div>
                    <div class="detail-section">
  ...
  runReport(reportName) {
    ...
    setTimeout(() => {
      const report = this.data.report_templates.find(
        (r) => r.name === reportName,
      );
  ...
  confirmScheduleReport(reportName) {
    ...
    this.closeModal();
    this.showToast(
      `${reportName} report scheduled ${frequency} starting ${startDate}`,
      "success",
    );
  }
  ...
  healthCheckDevice(deviceId) {
    ...
    setTimeout(() => {
      ...
      this.data.events.push({
        ...
        type: "Health Check",
        asset:
          device.assigned_assets.length > 0
            ? device.assigned_assets[0]
            : "System",
        device: deviceId,
      ...
      this.updateAllViews();
      this.showToast(
        `Health check completed! Device ${deviceId} status: ${newHealth}`,
        "success",
      );
    }, 2000);
  }
  ...
  unassignDevice(deviceId, assetId) {
    ...
    if (device.assigned_assets) {
      device.assigned_assets = device.assigned_assets.filter(
        (id) => id !== assetId,
      );
    ...
    if (asset.assigned_devices) {
      asset.assigned_devices = asset.assigned_devices.filter(
        (id) => id !== deviceId,
      );
    }
  ...
  validateDeviceSelection() {
    ...
    if (selectedDevices.length === 0) {
      feedback.textContent =
        "Select 2-3 devices to assign to this vehicle for optimal tracking";
      feedback.style.color = "var(--color-text-secondary)";
    } else if (selectedDevices.length === 1) {
      feedback.textContent =
        "Consider adding 1-2 more devices for comprehensive tracking";
